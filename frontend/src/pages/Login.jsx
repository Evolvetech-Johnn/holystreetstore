...
const Login = ({ setCurrentPage }) => {
  const { login, socialLogin, forgotPassword, loading, error } = useAuth();
  ...
  return (
    <div className="min-h-screen bg-dark-primary flex items-center justify-center p-4 relative overflow-hidden">
        ...
        <div className="absolute top-0 left-0 w-full h-full overflow-hidden z-0">
            <div className="absolute top-[-10%] left-[-10%] w-[50%] h-[50%] bg-primary-pink/10 rounded-full blur-[120px]"></div>
            <div className="absolute bottom-[-10%] right-[-10%] w-[50%] h-[50%] bg-primary-green/10 rounded-full blur-[120px]"></div>
        </div>
        ...
        <div className="relative z-10 w-full max-w-md bg-dark-secondary border border-gray-800 rounded-3xl p-8 md:p-12 shadow-2xl animate-fade-in">
            ...
            <div className="text-center mb-10">
                ...
                <p className="text-gray-400 text-sm">
                    {forgotMode 
                        ...
                        : 'Entre para acessar seus pedidos e drops exclusivos.'}
                </p>
            </div>
            ...
            {statusMsg && (
                <div className={`text-xs font-bold p-4 rounded-xl mb-6 text-center ${statusMsg.type === 'success' ? 'bg-green-500/10 border border-green-500/50 text-green-500' : 'bg-red-500/10 text-red-500'}`}>
                    {statusMsg.text}
                </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div>
                        ...
                        <div className="flex justify-end mt-2">
                            <button 
                                ...
                                className="text-[10px] font-bold uppercase tracking-wider text-gray-500 hover:text-primary-pink transition-colors"
                            >
                                Esqueci minha senha
                            </button>
                        </div>
                ...
                <button 
                    type="submit"
                    disabled={loading}
                    className={`${componentStyles.btnPrimary} w-full py-4 text-sm`}
                >
                    {loading ? 'Processando...' : (forgotMode ? 'Enviar Link' : 'Entrar')}
                </button>
            </form>

            <div className="mt-8">
                <div className="relative flex justify-center text-xs">
                    <span className="px-2 bg-dark-secondary text-gray-500 uppercase tracking-widest font-bold z-10">Ou entre com</span>
            ...
            <div className="mt-8 text-center">
                 {forgotMode ? (
                    <button 
                        ...
                        className="text-xs font-bold text-white hover:text-primary-pink transition-colors"
                    >
                        Voltar para o Login
                    </button>
                 ) : (
